...
dmodels_load_player_textures:
    type: task
    debug: false
    description: Load all required player texture data info.
    script:
    # =============== Clear out pre-existing data ===============
    - flag server dmodels_data.model_player_model_template_norm:!
    - flag server dmodels_data.model_player_model_template_slim:!
    - flag server dmodels_data.animations_player_model_template_norm:!
    - flag server dmodels_data.animations_player_model_template_slim:!
    # ============== Startup ===============
    - define template_data <script[dmodels_config].data_key[templates]||null>
    ...
    - if <[norm_data]> == null || <[slim_data]> == null || <[template_data]> == null:
      ...
      - stop
    - flag server dmodels_data.template_data.norm:<[norm_data]>
    - flag server dmodels_data.template_data.slim:<[slim_data]>
    ...
    - foreach <[tex_load_order]> as:tex_name:
        - foreach <[norm_models]> key:uuid as:model:
            - if <[tex_name]> == <[model.name]>:
                ...
                - flag server dmodels_data.model_player_model_template_norm.<[uuid]>:<[model]>
        - foreach <[slim_models]> key:uuid as:model:
            - if <[tex_name]> == <[model.name]>:
                - define model.type default
                - flag server dmodels_data.model_player_model_template_slim.<[uuid]>:<[model]>
